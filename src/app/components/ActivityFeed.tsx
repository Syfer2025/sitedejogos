...
export function ActivityFeed({ history, locale }: ActivityFeedProps) {
  ...
  return (
    <div className="rounded-3xl border border-white/5 bg-white/[0.02] backdrop-blur-3xl p-6 shadow-2xl h-full flex flex-col">
      <div className="flex flex-col h-full space-y-6 min-h-0">
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar min-h-0">
          <div className="grid gap-4">
            {history.map((entry) => (
              <div key={entry.id} className="group relative overflow-hidden rounded-2xl border border-slate-800/40 bg-slate-900/40 p-4 transition-all duration-300 hover:border-slate-700 hover:bg-slate-900/60 hover:shadow-lg">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  {/* Game Thumbnail */}
                  <div className="relative aspect-[16/10] h-20 shrink-0 overflow-hidden rounded-xl border border-slate-800 sm:h-16 md:h-20">
                    <Image
        ...
        {history.length > 5 && (
          <div className="shrink-0 pt-2">
            <button className="w-full rounded-2xl border border-slate-800 bg-slate-900/30 py-3 text-[11px] font-bold text-slate-500 transition-all hover:bg-slate-900/50 hover:text-slate-300">
