...
export const metadata: Metadata = {
  title: "Trending Games Online Free — Most Played HTML5 Games | Gasty Games",
  description: "The most popular free online games right now. Play trending HTML5 games instantly in your browser — no download, no install.",
  alternates: { canonical: "/trending" },
  openGraph: {
    title: "Trending Games — Gasty Games",
    description: "Most played free browser games right now.",
...
export default async function TrendingPage() {
  const games = await prisma.game.findMany({
    ...
    select: {
      ...
      category: true, featured: true, views: true,
    },
  });
  ...
  const structuredData = {
    ...
    "@type": "CollectionPage",
    name: "Trending Games — Gasty Games",
    ...
    <div className="h-full overflow-y-auto scrollbar-thin">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 border-b border-slate-800 pb-4">
          <h1 className="text-3xl font-bold text-white mb-2">Trending Games</h1>
          <p className="text-sm text-slate-400">
            The most played free browser games right now — ranked by total plays.
          </p>
        </div>

        <AdSlot label="Trending Top" slot={process.env.NEXT_PUBLIC_ADSENSE_SLOT_HOME_TOP} />
          ...
          {games.map((game, index) => (
            ...
            >
              <div className="relative aspect-video overflow-hidden bg-slate-950">
                ...
                {index < 3 && (
                  <span className="absolute top-2 left-2 rounded bg-amber-400/90 px-1.5 py-0.5 text-[9px] font-bold text-slate-950">
                    #{index + 1}
                  </span>
                )}
              </div>
              <div className="px-2.5 py-1.5">
                ...
                <span className="text-[10px] text-slate-500">{formatViews(game.views)} plays</span>
              </div>
            </Link>
          ))}
        </div>

        <AdSlot label="Trending Bottom" slot={process.env.NEXT_PUBLIC_ADSENSE_SLOT_HOME_CONTENT} />
