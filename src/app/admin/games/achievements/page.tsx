  ...
  async function handleImageUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!response.ok || !data?.url) {
        setFeedback({
          type: "error",
          message: data?.error ?? "Não foi possível enviar a thumb.",
        });
        return;
      }

      setForm((current) => ({ ...current, imageUrl: data.url ?? current.imageUrl }));
      setFeedback({
        type: "success",
        message: "Thumb enviada. Salve a conquista para confirmar.",
      });
    } catch {
      setFeedback({
        type: "error",
        message: "Falha inesperada ao enviar a thumb.",
      });
    } finally {
      setUploading(false);
      event.target.value = "";
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs text-slate-400">
            Crie novas conquistas, ajuste regras de desbloqueio e suba a thumb de cada badge.
          </p>
        </div>
      </header>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.05fr,1.35fr]">
        <form onSubmit={handleSubmit} className="space-y-4 rounded-3xl border border-slate-800 bg-slate-950/80 p-5">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-300">Thumb da conquista</label>
            <input
              type="text"
              value={form.imageUrl}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-[11px] text-slate-400">
              <label className="inline-flex cursor-pointer items-center rounded-full border border-slate-700 bg-slate-950/70 px-3 py-1.5 transition-colors hover:border-amber-500/60 hover:text-slate-100">
                ...
                />
                {uploading ? "Enviando thumb..." : "Enviar thumb"}
              </label>
